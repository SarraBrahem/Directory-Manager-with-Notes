...
public class CopyCommand implements Command {
	...
	public CopyCommand(Repertoire repertoire, CommandManager commandManager) {
		this.repertoire = repertoire;
		this.ner = repertoire.getLastFileUsed();
		this.commandManager = commandManager;
	}

	public CopyCommand(Repertoire repertoire, int ner, CommandManager commandManager) {
		this.repertoire = repertoire;
		this.ner = ner;
		this.commandManager = commandManager;
	}
	...
	public void execute() {
		if (ner < 0 || ner >= repertoire.numberOfFiles()) {
			...
			if (this.file.isDirectory()) {
				this.commandManager.showMessage("Le répertoire " + this.file.getName() + " est prêt à être copié.");
			} else {
				this.commandManager.showMessage("Le fichier " + this.file.getName() + " est prêt à être copié.");
			}
		}
	}
